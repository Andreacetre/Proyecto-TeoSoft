      ...
      if (req.file) {
        try {
          ...
          if (existingProducto.FotosProducto) {
            try {
              const publicId = existingProducto.FotosProducto.split("/").pop().split(".")[0]
              await deleteFromCloudinary(publicId)
            } catch (deleteError) {
              console.error("Error al eliminar imagen anterior de Cloudinary:", deleteError)
              // Continuar incluso si hay error al eliminar la imagen anterior
            }
          }
    ...
    try {
      ...
      if (existingProducto.FotosProducto) {
        try {
          ...
          // Continuar con la eliminación del producto incluso si hay error al eliminar la imagen
        }
      }
      ...
      if (req.file) {
        try {
          ...
          if (variant.FotosProducto) {
            try {
              const publicId = variant.FotosProducto.split("/").pop().split(".")[0]
              await deleteFromCloudinary(publicId)
            } catch (deleteError) {
              console.error("Error al eliminar imagen anterior de Cloudinary:", deleteError)
              // Continuar incluso si hay error al eliminar la imagen anterior
  ...
  deleteVariant: async (req, res) => {
    try {
      ...
      if (variante.FotosProducto) {
        try {
          const publicId = variante.FotosProducto.split("/").pop().split(".")[0]
          await deleteFromCloudinary(publicId)
        } catch (deleteError) {
          console.error("Error al eliminar imagen de Cloudinary:", deleteError)
          // Continuar con la eliminación de la variante incluso si hay error al eliminar la imagen
        }
      }
